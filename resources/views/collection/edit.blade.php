                    ...
                    <div class="max-w-xl mx-auto">
                        <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                            ...
                            <form method="POST" action="{{ route('collection.update', $collection) }}">
                                ...
                                <div class="mb-4">
                                    <label for="name" class="block text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Collection Name') }}</label>
                                    <input id="name" name="name" type="text" value="{{ $collection->name }}" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 block w-full" required>
                                </div>

                                <div class="flex justify-end space-x-2">
                                    <button onclick="event.preventDefault(); location.href='{{ route('collection.index', $collection) }}'" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 mr-2">
                                        {{ __('Cancel') }}
                                    </button>
